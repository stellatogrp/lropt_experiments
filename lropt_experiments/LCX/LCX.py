...
def min_max(eps,alpha,data,datamax,test,validate):
    ...
    x = np.ones(n)/n
    u_set = []
    obj,uval = gen_problem(Gamma,eps,data,x)
    u_set.append(uval)
    ...
    while abs(objnew - obj) >= 1e-3:
        if outeriter > max_iter_outer:
            ...
            break
        outeriter+=1
        obj,uval = gen_problem(Gamma,eps,data,x.value)
        u_set.append(uval)
        prob, x, t = create_max(u_set)
        prob.solve()
        objnew = prob.objective.value
    eval, prob_vio = calc_eval(x.value, t.value,test)
    eval_vali, prob_vali = calc_eval(x.value, t.value,validate)
    return eval, prob_vio, eval_vali, prob_vali
...
@hydra.main(config_path="/Users/irina.wang/Desktop/Princeton/Project2/lropt_experiments/lropt_experiments/LCX/configs",config_name = "lcx1.yaml", version_base = None)
def main_func(cfg):
