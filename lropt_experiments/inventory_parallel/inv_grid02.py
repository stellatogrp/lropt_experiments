...
def gen_demand_varied(sig,mu,d,N,seed=399):
    ...
    np.random.seed(seed)
    for i in range(N):
        d_train = np.random.multivariate_normal(d - 0.1*mu[i],sig[i]+0.2*np.eye(d.shape[0]))
...
def inv_exp(cfg,hydra_out_dir,seed):
    ...
    try: 
        ...
        try: 
            ...
            dir = '/scratch/gpfs/iywang/lropt_revision/lropt_experiments/inv_results/2025-05-04/2/'+hydra_out_dir[-1]
