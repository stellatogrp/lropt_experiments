...
def gen_demand_varied(d, cov, N, cval=None, pval=None, hval=None, seed=399):
    ...
    if cval is not None and pval is not None and hval is not None:
        ...
        d_train = d_train + 5*(cval_norm + pval_norm + hval_norm)
...
def inv_exp(seed):
    ...
    try: 
        ...
        objective = C
        cons = [c_param@q + cqv@d + cp.sum(y + y_var@d) + cp.sum(u + u_var@d) + cp.sum(z) - C]
        constraints = []
        ...
        for time in range(T):
            ...
            cons += [-(y + y_var@d)[time] + h_param[time]*x_init + h_param[time]*cp.sum(q[:(time+1)]) + hqvd[time, :]@d - h_aux[time]*cp.sum(d[:(time+1)]) ]
            cons+= [-(y + y_var@d)[time] -p_param[time]*x_init - p_param[time]*cp.sum(q[:(time+1)]) - pqvd[time, :]@d + p_aux[time]*cp.sum(d[:(time+1)]) ]
        ...
        prob = lropt.RobustProblem(cp.Minimize(objective), constraints, eval_exp = c_param@q + cqv@d + cp.sum(y + y_var@d) + cp.sum(u + u_var@d) + cp.sum(z))
        ...
        settings.test_frequency = 3000
        settings.validate_frequency = 3000
