...
def gen_demand_varied(d, cov, N, cval=None, pval=None, hval=None, seed=399):
    ...
    if cval is not None and pval is not None and hval is not None:
        ...
        d_train = d_train + 50*(cval_norm + pval_norm + hval_norm)
...
def inv_exp(cfg,hydra_out_dir,seed):
    ...
    try: 
        ...
        for time in range(T):
            constraints +=[Vmin -x_init - cp.sum((q+q_var@d)[:time+1]) + cp.sum(d[:(time+1)]) <=0]
            constraints +=[x_init + cp.sum((q+q_var@d)[:(time+1)]) - cp.sum(d[:(time+1)])- Vmax <=0]
            ...
            constraints += [-(q+q_var@d)[time] <=0]
            constraints += [(q+q_var@d - Qmax)[time] <=0]
