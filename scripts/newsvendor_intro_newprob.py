...
def loss(t, x, y1, d1, y2, d2, k_tch, p_tch, alpha, data, l=5, quantile=0.95, target=1.):
    ...
    # sums = torch.mean(torch.maximum(k_tch[1]*x[1] + torch.maximum(-data[:, 0]*p_tch[0], - p_tch[0]*x[0]) +
    #                   torch.maximum(-data[:, 1]*p_tch[1], -p_tch[1]*x[1]) - t,  torch.tensor(0., requires_grad=True)))
    ...
    probn = torch.sum((torch.maximum(k_tch[1]*x[1] + torch.maximum(-data[:, 0]*p_tch[0], - p_tch[0]*x[0]) + torch.maximum(
        -data[:, 1]*p_tch[1], -p_tch[1]*x[1]) - t,  torch.tensor(0., requires_grad=True))) >= torch.tensor(0.0001))/Nsample
    sums = sums/(1-quantile) + alpha
    objective = torch.mean(k_tch[1]*x[1] + torch.maximum(-data[:, 0]*(p_tch[0]), - x[0]*(
...
constraints = [k >= 0, p >= 0]
constraints += [k[1]*x_r[1] + y1 + d1 *
                np.array([1, 0])@u + y2 + d2*np.array([0, 1])@u <= t]
constraints += [cp.maximum(-p[0]*x_r[1], -p[0] *
...
plt.savefig("news_objective_vs_epsilon_newprob.pdf", bbox_inches='tight')

# plot statistics
plot_coverage(dfgrid, dfgrid2, "news_newprob", ind_1=(0, 500), ind_2=(0, 500))
plot_iters(df1, "news_newprob", steps=800, logscale=1)
...
# plt.xscale("log")
# plt.ylim([-2, 1])
plt.vlines(ymin=-10, ymax=1, x=prob_list[0], linestyles=":",
           color="tab:red", label="Reference $\hat{\eta}$")
for i in prob_list[1:]:
    plt.vlines(ymin=-10, ymax=1, x=i, linestyles=":", color="tab:red")
plt.legend()
plt.savefig("news_objective_vs_violations_newprob.pdf", bbox_inches='tight')
...
# init_set = np.zeros((num_p,num_p))
    ...
    for i in range(num_p):
        for j in range(num_p):
            ...
            for scene in range(num_scenarios):
                g_level_learned[ind][scene][i, j] = scenarios[scene][0][1]*x_opt_learned[ind][scene][1] + np.maximum(- scenarios[scene][1][0] * x_opt_learned[ind][scene][0], - scenarios[scene][1][0] * u_vec[0]) + np.maximum(
                ...
                g_level_base[ind][scene][i, j] = scenarios[scene][0][1]*x_opt_base[ind][scene][1] + np.maximum(- scenarios[scene][1][0] * x_opt_base[ind][scene][0], - scenarios[scene][1][0] * u_vec[0]) + np.maximum(
...
plot_contours_line(x, y, init_set, g_level_base, prob_list,
                   num_scenarios, train, "news_intro_newprob", standard=True)
plot_contours_line(x, y, fin_set, g_level_learned, prob_list,
                   num_scenarios, train, "news_intro_newprob", standard=False)
