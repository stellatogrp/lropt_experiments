...
def plot_coverage_all(df_standard,df_reshape,dfs,title,ind_1 = (0,100), ind_2 = (0,100), logscale = True, legend = False, zoom = False):
    ...
    ax.plot(np.mean(np.vstack(df_standard['Probability_violations_test']),axis = 1)[beg1:end1], np.mean(np.vstack(df_standard['Test_val']),axis = 1)[beg1:end1], color="tab:blue", label=r"Mean-Var set")
    ax.fill(np.append(np.quantile(np.vstack(df_standard['Probability_violations_test']),0.1,axis = 1)[beg1:end1],np.quantile(np.vstack(df_standard['Probability_violations_test']),0.9,axis = 1)[beg1:end1][::-1]), np.append(np.quantile(np.vstack(df_standard['Test_val']),0.1,axis = 1)[beg1:end1],np.quantile(np.vstack(df_standard['Test_val']),0.9,axis = 1)[beg1:end1][::-1]), color="tab:blue", alpha=0.2)

    ax.plot(np.mean(np.vstack(df_reshape['Probability_violations_test']),axis = 1)[beg2:end2], np.mean(np.vstack(df_reshape['Test_val']),axis = 1)[beg2:end2], color="tab:orange", label=r"Reshaped set")
    ax.fill(np.append(np.quantile(np.vstack(df_reshape['Probability_violations_test']),0.1,axis = 1)[beg2:end2],np.quantile(np.vstack(df_reshape['Probability_violations_test']),0.9,axis = 1)[beg2:end2][::-1]), np.append(np.quantile(np.vstack(df_reshape['Test_val']),0.1,axis = 1)[beg2:end2],np.quantile(np.vstack(df_reshape['Test_val']),0.9,axis = 1)[beg2:end2][::-1]), color="tab:orange", alpha=0.2)
    ...
    ax2.plot(np.mean(np.vstack(df_standard['Coverage_test']),axis = 1)[beg1:end1], np.mean(np.vstack(df_standard['Probability_violations_test']),axis = 1)[beg1:end1], color="tab:blue", label=r"Mean-Var set")

    ax2.plot(np.mean(np.vstack(df_reshape['Coverage_test']),axis = 1)[beg2:end2], np.mean(np.vstack(df_reshape['Probability_violations_test']),axis = 1)[beg2:end2], color="tab:orange", label=r"Reshaped set",alpha = 0.8)
    if dfs:
        for i in range(5):
...
# Train A and b
result = prob.train(lr=0.0001, num_iter=2000, momentum=0.8, optimizer="SGD",
...
# Grid search epsilon
result4 = prob.grid(epslst=np.linspace(0.01, 4, 500), init_A=init,
                    init_b=init_bval, seed=s,
                    init_alpha=0., test_percentage=test_p)
dfgrid = result4.df

result5 = prob.grid(epslst=np.linspace(0.01, 4, 500), init_A=A_fin, init_b=b_fin, seed=s,
                    init_alpha=0., test_percentage=test_p)
dfgrid2 = result5.df

plot_coverage_all(dfgrid,dfgrid2,None, "Portlinear",ind_1=(0,400),ind_2=(0,400), logscale = False, zoom = False,legend = True)

plot_iters(df, result.df_test, "Port", steps = 10000,logscale = 1)

eps_list = np.linspace(0.01, 4, 500)
...
plt.fill(np.append(np.quantile(np.vstack(dfgrid['Probability_violations_test']), 0.25, axis=1)[beg1:end1], np.quantile(np.vstack(dfgrid['Probability_violations_test']), 0.75, axis=1)[beg1:end1][::-1]), np.append(
...
plt.fill(np.append(np.quantile(np.vstack(dfgrid2['Probability_violations_test']), 0.25, axis=1)[beg2:end2], np.quantile(np.vstack(dfgrid2['Probability_violations_test']), 0.75, axis=1)[beg2:end2][::-1]), np.append(
